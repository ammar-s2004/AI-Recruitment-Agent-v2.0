...
function Interview() {
  ...
  useEffect(() => {
    const checkAccess = async () => {
      // 1. Get current session (optional for candidates)
      const { data: { session } } = await supabase.auth.getSession();

      // If user is logged in, auto-fill details and capture IDs
      if (session?.user) {
        setUserName(session.user.user_metadata?.full_name || "");
        setUserEmail(session.user.email || "");
        setCandidateId(session.user.id);

        // Also fetch CV from users table
        ...
        if (userData?.cv_file_path) {
          setResumeUrl(userData.cv_file_path);
        }

        // Check if there's an invited candidate email restriction
        if (interviewData?.invited_candidate_email) {
          if (interviewData.invited_candidate_email !== session.user.email) {
            // Email mismatch - deny access
        ...
        // If no session but interview has invited email restriction, require login
        if (interviewData?.invited_candidate_email) {
          toast.warning('Please log in to access this interview');
          router.push('/signin');
          return;
        }
      }

      // No login required for candidates without email restriction
  ...
  const GetInterviewDetails = async () => {
    ...
    try {
      const { data: Interviews, error } = await supabase
        .from('interviews')
        .select('useremail, jobposition, jobdescription, duration, type, questionlist, invited_candidate_email')
  ...
  const onJoinInterview = async () => {
    ...
    try {
      const newInterviewInfo = {
        ...
        interviewer_email: interviewData?.useremail // Recruiter's email
      };
