          ...
          {results.length === 0 ? (
            ...
            <div className="divide-y">
              {results.map((result) => {
                ...
                const summary = feedback?.summary || feedback?.summery || '';
                const strengths = feedback?.strengths || [];
                const weaknesses = feedback?.weaknesses || [];
                const improvementPlan = feedback?.improvement_plan || feedback?.improvementPlan || [];
                const recommendation = feedback?.Recommendation || '';
                const recommendationMsg = feedback?.RecommendationMessage || feedback?.["Recommendation Message"] || '';
                  ...
                  <div key={result.id} className="bg-white rounded-lg shadow-md p-6 mb-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        {strengths.length > 0 && (
                          <div className="bg-green-50 p-3 rounded-lg border border-green-100">
                            <h5 className="text-xs font-bold text-green-800 mb-2 uppercase tracking-wider">Strengths</h5>
                            <ul className="space-y-1">
                              {strengths.map((str, i) => (
                                <li key={i} className="text-xs text-green-700 flex gap-1.5">
                                  <span className="mt-1 w-1 h-1 bg-green-400 rounded-full flex-shrink-0" />
                                  {str}
                                </li>
                              ))}
                            </ul>
                        ...
                        {weaknesses.length > 0 && (
                          <div className="bg-red-50 p-3 rounded-lg border border-red-100">
                            <h5 className="text-xs font-bold text-red-800 mb-2 uppercase tracking-wider">Areas for Improvement</h5>
                            <ul className="space-y-1">
                    ...
                    {improvementPlan.length > 0 && (
                      <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-100">
                        <h4 className="text-xs font-bold text-blue-800 mb-2 uppercase tracking-wider">Actionable Improvement Plan</h4>
                    ...
                    {recommendation && (
                      <div className="mb-4">
                        ...
                        <div className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${recommendation.toLowerCase().includes('recommended')
