...
function QuestionList({ formData, onCreateLink }) {
  ...
  const [cvStatus, setCvStatus] = useState(null); // null | 'found' | 'not_found'
  ...
  useEffect(() => {
    const checkCandidateCV = async () => {
      if (!formData?.candidateEmail) return;
      const { data } = await supabase
        .from('users')
        .select('cv_file_path')
        .eq('email', formData.candidateEmail)
        .single();
      setCvStatus(data?.cv_file_path ? 'found' : 'not_found');
    };
    checkCandidateCV();
  ...
  const onFinish = async () => {
    ...
    // Generate unique 8-character interview code
    ...
    const interview_code = generateInterviewCode();

    console.log("Final question list being saved:", questionList);
    console.log("Form data being saved:", formData);
    console.log("Generated interview code:", interview_code);
    ...
    try {
      ...
      // Then create the interview with code
        ...
        .insert([
          {
            ...
            invited_candidate_email: formData.candidateEmail || null,
    ...
    <div>
        ...
        <div className="flex flex-col items-center gap-4 mt-10">
          ...
          <div className="p-5 bg-blue-50 rounded-xl border border-gray-100 flex flex-col gap-2 items-center text-center">
            ...
            <p className="text-sm text-gray-600">
              {cvStatus === 'found'
                ? 'Analyzing candidate resume and crafting tailored, rigorous questions...'
                : 'Our AI is crafting rigorous questions based on the job requirements...'}
            </p>
          </div>
        </div>
      )}
      ...
      {!loading && questionList && questionList.interviewQuestions && (
        <div className="mt-10">
          ...
          {/* CV Status Banner */}
          {formData?.candidateEmail && cvStatus && (
            <div className={`mb-4 p-4 rounded-lg border flex items-start gap-3 ${cvStatus === 'found'
                ? 'bg-green-50 border-green-200'
                : 'bg-amber-50 border-amber-200'
              }`}>
              <span className="text-xl">{cvStatus === 'found' ? '✅' : '⚠️'}</span>
              <div>
                <p className={`font-semibold text-sm ${cvStatus === 'found' ? 'text-green-800' : 'text-amber-800'
                  }`}>
                  {cvStatus === 'found'
                    ? `Resume found for ${formData.candidateEmail}`
                    : `No resume uploaded by ${formData.candidateEmail}`}
                </p>
                <p className={`text-xs mt-0.5 ${cvStatus === 'found' ? 'text-green-700' : 'text-amber-700'
                  }`}>
                  {cvStatus === 'found'
                    ? 'Questions have been tailored to their specific background and experience.'
                    : 'General role-based questions generated. Ask the candidate to upload their resume for tailored questions.'}
                </p>
              </div>
            </div>
          )}
